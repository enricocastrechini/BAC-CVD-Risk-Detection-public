...
from sklearn.metrics import f1_score, recall_score, precision_score, accuracy_score, roc_auc_score, confusion_matrix, matthews_corrcoef
...
class EvaluateModel:
    ...
    def binary_eval(self, y, y_prob, thresh_search, best_thresh):
        ...
        if check_imbalance >= 2:
            ...
            _best_thresh, scores_dict['aupr_best_pr'], scores_dict['aupr_best_rec'] = self.get_threshold(scores_dict['aupr_pr'], scores_dict['aupr_rec'], scores_dict['aupr_thresholds'])
        else:
            scores_dict['fpr'], scores_dict['tpr'], scores_dict['roc_thresh'] = roc_curve(y_true=y, y_score=y_prob)
            _best_thresh, scores_dict['roc_best_fpr'], scores_dict['roc_best_tpr'] = self.get_threshold(scores_dict['fpr'], scores_dict['tpr'], scores_dict['roc_thresh'])
    ...
    def get_threshold(self, vector1, vector2, thresholds):
        ...
        for idx in range(n_thresh):
            if vector1[idx] == 0 and vector2[idx] == 0:
                ...
                curr_f1 = (2 * vector1[idx] * vector2[idx]) / (vector1[idx] + vector2[idx])
