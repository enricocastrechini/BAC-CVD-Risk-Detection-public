...
class BreastSegmenter:
    def __init__(self, scale_factor=0.25, threshold=3900, felzenzwalb_scale=0.15):
        self.scale_factor = scale_factor
        self.threshold = threshold
        self.felzenzwalb_scale = felzenzwalb_scale

    def __call__(self, im):
        # return self.breast_segment(im)
        # set threshold to remove artifacts around edges
        im_thres = im.copy()
        im_thres[im_thres > self.threshold] = 0
        ...
        # rescale and filter aggressively, normalize
        im_small = rescale(im_thres, self.scale_factor)
        im_small_filt = median(im_small, disk(50))
        # this might not be helping, actually sometimes it is
        im_small_filt = equalize_hist(im_small_filt)

        # run mr. felzenzwalb
        segments = felzenszwalb(im_small_filt, scale=self.felzenzwalb_scale)
        segments += 1  # otherwise, labels() would ignore segment with segment=0
        ...
        # exclude thresholded area (artifacts) in mask, too
        im_mask[im > self.threshold] = False
