...
class SparseEncoder(nn.Module):
    def __init__(self, cnn, input_size=None, input_width=None, input_height=None, sbn=False, verbose=False):
        ...
        print(f"SparseEncoder initialized with input size: {(input_width, input_height)}, downsample ratio: {self.downsample_ratio}")


    @staticmethod
    def dense_model_to_sparse(m: nn.Module, verbose=False, sbn=False):
        print(f"Converting {type(m)} to sparse")
        oup = m
    ...
    def forward(self, x):
        print(f"Forward pass of SparseEncoder, input shape: {x.shape}")
        # return self.sp_cnn(x, hierarchical=True)
        result = self.sp_cnn(x, hierarchical=True)
        print(f"Output shapes of hierarchical encoded features: {[r.shape for r in result]}")
        return result
