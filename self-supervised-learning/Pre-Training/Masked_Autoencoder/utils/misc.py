...
def time_str(for_dirname=False):
    return datetime.datetime.now(tz=pytz.timezone('Europe/Rome')).strftime('%m-%d_%H-%M-%S' if for_dirname else '[%m-%d %H:%M:%S]')
...
def save_checkpoint_for_finetune(save_to, args, sp_cnn_state, epoch, last_loss, best_loss, frequency=10):
    """
    Save checkpoint for finetuning.
    
    Args:
        save_to (str): The base filename to save the checkpoint.
        args: The arguments containing model and experiment details.
        sp_cnn_state (dict): The state of the sparse CNN model.
        epoch (int): The current epoch number.
        last_loss (float): The loss of the current epoch.
        best_loss (float): The best loss seen so far.
        frequency (int): How often (in epochs) to save checkpoints. Default is 10.
    """
    # Save every `frequency` epochs
    # if epoch % frequency == 0 or last_loss < best_loss:
    if epoch % frequency == 0:
        epoch_number = f'Epoch_{epoch}_'
        loss_str = f'Loss_{last_loss:.4f}_'
        checkpoint_path = os.path.join(args.exp_dir, epoch_number + loss_str + save_to)
...
def initialize_weight(init_weight: str, model_without_ddp, input_height: int, input_width: int):
    # Use some checkpoint as model weight initialization; ONLY load model weights
    if len(init_weight):
        checkpoint = torch.load(init_weight, 'cpu')
        
        # Update the 'norm_black' buffer in the checkpoint to match the input dimensions
        # if 'norm_black' in checkpoint.get('module', checkpoint):
        #     checkpoint['norm_black'] = torch.zeros(1, 3, input_height, input_width)
        #     print(f'[initialize_weight] Updated norm_black to shape: {checkpoint["norm_black"].shape}')
        # else:
        #     print('[initialize_weight] norm_black not found in checkpoint')
