...
def main_pt():
    ...
    print(f"Loaded dataset with {len(dataset_train)} samples, input size: {(args.input_width, args.input_height)}")
    ...
    misc.initialize_weight(args.init_weight, model_without_ddp, args.input_height, args.input_width)
    ...
    if ep_start >= args.ep: # load from a complete checkpoint file
        ...
        for ep in range(ep_start, args.ep):
            ...
            misc.save_checkpoint_for_finetune(f'{args.model}_1kpretrained.pth', args, model_without_ddp.sparse_encoder.sp_cnn.state_dict(), ep, last_loss, best_loss)
...
def pre_train_one_ep(ep, args: arg_util.Args, tb_lg: misc.TensorboardLogger, itrt_train, iters_train, model: DistributedDataParallel, optimizer):
    ...
    for it, (inp, _) in enumerate(me.log_every(iters_train, itrt_train, 3, header)):
        print(f"Epoch {ep}, Iteration {it}, Input shape: {inp.shape}")
        # adjust lr and wd
