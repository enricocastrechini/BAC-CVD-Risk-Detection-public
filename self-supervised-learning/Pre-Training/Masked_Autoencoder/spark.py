...
class SparK(nn.Module):
    def __init__(
        ...
        super().__init__()
        input_width, input_height, downsample_ratio = sparse_encoder.input_width, sparse_encoder.input_height, sparse_encoder.downsample_ratio
        self.downsample_ratio = downsample_ratio
        self.fmap_width = input_width // downsample_ratio
        self.fmap_height = input_height // downsample_ratio
        self.mask_ratio = mask_ratio
        self.len_keep = round(self.fmap_width * self.fmap_height * (1 - mask_ratio))
        ...
        print(f"SparK initialized with input width: {input_width}, input height: {input_height}, mask ratio: {self.mask_ratio}")
        ...
        e_widths: List[int]
        for i in range(self.hierarchy):  # from the smallest feat map to the largest
            ...
            if i == 0 and e_width == d_width:
                densify_proj = nn.Identity()  # NOTE: ConvNeXT-S uses this because its width is 768, which equals the decoder's width
                ...
                print(f'[SparK.__init__, densify {i+1}/{self.hierarchy}]: densify_proj(ksz={kernel_size}, #params={sum(x.numel() for x in densify_proj.parameters()) / 1e6:.2f}M)')
        ...
        # self.register_buffer('norm_black', torch.zeros(1, 3, input_height, input_width))
        self.vis_active = self.vis_active_ex = self.vis_inp = self.vis_inp_mask = ...
    
    def mask(self, B: int, device, generator=None):
        fw, fh = self.fmap_width, self.fmap_height
        print(f"Generating mask, fmap_width: {fw}, fmap_height: {fh}")
        ...
        mask = torch.zeros(B, fh * fw, dtype=torch.bool, device=device).scatter_(dim=1, index=idx, value=True).view(B, 1, fh, fw)
        print(f"Mask shape: {mask.shape}")
        return mask
    
    def forward(self, inp_bchw: torch.Tensor, active_b1ff=None, vis=False):
        print(f"Forward pass of SparK, input shape: {inp_bchw.shape}")
        
        # Step 1: Mask
        if active_b1ff is None:  # random mask
            active_b1ff: torch.BoolTensor = self.mask(inp_bchw.shape[0], inp_bchw.device)  # (B, 1, fh, fw)
        print(f"Active mask shape: {active_b1ff.shape}")

        encoder._cur_active = active_b1ff  # (B, 1, fh, fw)
        active_b1hw = active_b1ff.repeat_interleave(self.downsample_ratio, 2).repeat_interleave(self.downsample_ratio, 3)  # (B, 1, H, W)
        
        # Ensure active_b1hw has the same number of channels as inp_bchw
        active_bchw = active_b1hw.repeat(1, inp_bchw.size(1), 1, 1)  # Repeat for each channel
        masked_bchw = inp_bchw * active_bchw
        print(f"Masked input shape: {masked_bchw.shape}")

        
        # Step 2: Encode - get hierarchical encoded sparse features (a list containing feature maps at multiple scales)
        fea_bcffs: List[torch.Tensor] = self.sparse_encoder(masked_bchw)
        fea_bcffs.reverse()  # Reverse to process from the smallest feature map to the largest
        print(f"Encoded feature shapes: {[f.shape for f in fea_bcffs]}")

        
        # Step 3: Densify - get hierarchical dense features for decoding
        cur_active = active_b1ff  # (B, 1, fh, fw)
        to_dec = []
        for i, bcff in enumerate(fea_bcffs):  # Process from the smallest feature map to the largest
            if bcff is not None:
                print(f"Densifying feature map {i}, shape: {bcff.shape}")
            ...
            to_dec.append(bcff)
            cur_active = cur_active.repeat_interleave(2, dim=2).repeat_interleave(2, dim=3)  # Dilate the mask map, from (B, 1, fh, fw) to (B, 1, H, W)
        
        # Step 4: Decode and reconstruct
        rec_bchw = self.dense_decoder(to_dec)
        print(f"Reconstructed output shape: {rec_bchw.shape}")

        # Normalize the input and reconstruction
        inp, rec = self.patchify(inp_bchw), self.patchify(rec_bchw)  # inp and rec: (B, L = fh*fw, N = C*downsample_ratio**2)
        # mean = inp.mean(dim=-1, keepdim=True)
        # var = (inp.var(dim=-1, keepdim=True) + 1e-6) ** 0.5
        # inp = (inp - mean) / var
        l2_loss = ((rec - inp) ** 2).mean(dim=2, keepdim=False)  # (B, L, C) ==mean==> (B, L)
        
        non_active = active_b1ff.logical_not().int().view(active_b1ff.shape[0], -1)  # (B, 1, fh, fw) => (B, L)
    ...
    def patchify(self, bchw):
        ...
        fh, fw = self.fmap_height, self.fmap_width
        print(f"Patchifying input, shape: {bchw.shape}, fmap_height: {fh}, fmap_width: {fw}")
        B, C = bchw.shape[:2]
        bchw = bchw.reshape(shape=(B, C, fh, p, fw, p))
        bchw = torch.einsum('bchpwq->bhwpqc', bchw)
        bln = bchw.reshape(shape=(B, fh * fw, C * p ** 2))  # (B, fh*fw, 3*downsample_ratio**2)
        print(f"Patchified output shape: {bln.shape}")
        return bln
    
    def unpatchify(self, bln):
        p = self.downsample_ratio
        fh, fw = self.fmap_height, self.fmap_width
        print(f"Unpatchifying input, shape: {bln.shape}, fmap_height: {fh}, fmap_width: {fw}")
        B, C = bln.shape[0], bln.shape[-1] // p ** 2
        bln = bln.reshape(shape=(B, fh, fw, p, p, C))
        bln = torch.einsum('bhwpqc->bchpwq', bln)
        bchw = bln.reshape(shape=(B, C, fh * p, fw * p))
        print(f"Unpatchified output shape: {bchw.shape}")
        return bchw
    ...
    def get_config(self):
        return {
            ...
            # enc
            'sparse_encoder.input_width': self.sparse_encoder.input_width,
            'sparse_encoder.input_height': self.sparse_encoder.input_height,
            # dec
